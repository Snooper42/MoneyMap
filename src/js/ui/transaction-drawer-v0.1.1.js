/* MoneyMap transaction drawer v0.1.1 — polished desktop and mobile editor */
(function(){
  'use strict';

  if(window.__mmTransactionDrawerV011) return;
  window.__mmTransactionDrawerV011 = true;
  ...
  function txKind(t){ return (t && n(t.amount)>0) ? 'income' : 'spend'; }
  function commonCategories(kind){
    return kind==='income' ? ['Income','Savings','Investments','Other'] : ['Groceries','Dining','Coffee','Gas','Bills','Shopping','Subscriptions','Transportation','Transfers','Other'];
  }
  ...
  window.setTxTypeV011=function(kind){
    ...
    if(cat && (!cat.value || (kind==='income' && !['Income','Savings','Investments','Other'].includes(cat.value)) || (kind==='spend' && ['Income','Savings','Investments'].includes(cat.value)))) cat.value=kind==='income'?'Income':'Other';
    var chips=document.getElementById('mm11CategoryChips');
    if(chips) chips.innerHTML=commonCategories(kind).map(function(c){ return '<button type="button" class="mm11-cat-chip'+((cat?.value||'')===c?' selected':'')+'" data-cat="'+esc(c)+'" onclick="setQuickCategoryV011(\''+js(c)+'\')">'+esc(c)+'</button>'; }).join('');
    updateTxPreviewV011();
  };
  window.setQuickCategoryV011=function(cat){ setField('qaCat',cat); selectedCategoryButtons(); updateTxPreviewV011(); };
  window.updateTxPreviewV011=function(){
    var kind=selectedKind();
    var raw=cents(document.getElementById('qaAmount')?.value||0);
    var signed=kind==='spend'?-Math.abs(raw):Math.abs(raw);
  ...
  window.toggleTransferV011=function(){
    var sw=document.getElementById('qaHidden');
    if(!sw) return;
    sw.classList.toggle('on');
    var on=sw.classList.contains('on');
    if(on){ setField('qaCat','Transfers'); window.setTxTypeV011('spend'); }
    selectedCategoryButtons(); updateTxPreviewV011();
  };
  ...
  function transactionMarkup(data){
    ...
    var cat=t.category || (kind==='income'?'Income':'Other');
    var acct=t.account || 'General';
    var hidden=!!t.hidden || cat==='Transfers';
    var chips=commonCategories(kind).map(function(c){ return '<button type="button" class="mm11-cat-chip'+(cat===c?' selected':'')+'" data-cat="'+esc(c)+'" onclick="setQuickCategoryV011(\''+js(c)+'\')">'+esc(c)+'</button>'; }).join('');
    var avatar=esc(String(desc||'?').trim().slice(0,1).toUpperCase()||'?');
    return '<div class="mm11-tx-editor">' +
      '<section class="mm11-tx-preview"><div class="mm11-tx-avatar">'+avatar+'</div><div class="mm11-tx-preview-copy"><div id="mm11TxPreviewTitle">'+esc(desc||'Transaction')+'</div><span id="mm11TxPreviewMeta">'+esc(cat)+' · '+esc(acct)+' · '+esc(dateText(date))+'</span></div><strong id="mm11TxPreviewAmount" class="mm11-tx-preview-amount '+(kind==='spend'?'bad':'good')+'">'+esc(fmt(kind==='spend'?-Math.abs(n(t.amount)):Math.abs(n(t.amount))))+'</strong></section>' +
      '<section class="card drawer-form-card mm11-tx-form-card"><div class="tx-type-toggle mm11-type-toggle" role="group" aria-label="Transaction type"><button type="button" class="btn btn-small'+(kind==='spend'?' selected':'')+'" data-tx-type="spend" onclick="setTxTypeV011(\'spend\')">Spend</button><button type="button" class="btn btn-small'+(kind==='income'?' selected':'')+'" data-tx-type="income" onclick="setTxTypeV011(\'income\')">Income</button></div>' +
      '<div class="mm11-form-grid"><div><label>Amount</label><div class="money-input mm11-money-input"><span>$</span><input class="input" id="qaAmount" type="number" inputmode="decimal" step="0.01" min="0" value="'+esc(amountInputValue(t.amount))+'" placeholder="0.00" autocomplete="off" oninput="updateTxPreviewV011()"></div></div><div><label>Date</label><input class="input" id="qaDate" type="date" value="'+esc(date)+'" onchange="updateTxPreviewV011()"></div></div>' +
      '<div class="form-field"><label>Description</label><input class="input" id="qaDesc" data-autofocus value="'+esc(desc)+'" placeholder="Merchant, payee, or note" autocapitalize="words" autocorrect="off" autocomplete="off" oninput="updateTxPreviewV011()"></div>' +
      '<div class="mm11-form-grid"><div><label>Category</label><input class="input" id="qaCat" list="categoryList" value="'+esc(cat)+'" autocomplete="off" oninput="updateTxPreviewV011()"><div id="mm11CategoryChips" class="mm11-category-chips">'+chips+'</div></div><div><label>Account</label><input class="input" id="qaAcct" value="'+esc(acct)+'" autocomplete="off" oninput="updateTxPreviewV011()"></div></div>' +
      '<button type="button" class="toggle mm11-transfer-toggle" onclick="toggleTransferV011()"><div><b>Mark as transfer</b><br><span class="muted">Hides this item from spend, budgets, and charts.</span></div><span class="switch '+(hidden?'on':'')+'" id="qaHidden" aria-label="Hide as transfer"></span></button>' +
  ...
  window.saveQuickTransaction=function(id){
    ...
    var kind=selectedKind();
    var raw=cents(document.getElementById('qaAmount')?.value||0);
    var amount=kind==='spend'?-Math.abs(raw):Math.abs(raw);
    var desc=(document.getElementById('qaDesc')?.value||'').trim() || 'Manual transaction';
    var hidden=document.getElementById('qaHidden')?.classList.contains('on') || false;
