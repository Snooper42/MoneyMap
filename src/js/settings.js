...
function renderSettings(){
  ...
  const build=document.getElementById('appBuildLabel'); if(build) build.textContent=APP_BUILD_ID||window.MoneyMapConfig?.buildId||'v0.9.6';
...
function openDrawer(type,data=null){ const d=document.getElementById('drawer'), title=document.getElementById('drawerTitle'), sub=document.getElementById('drawerSub'), body=document.getElementById('drawerBody'); d.classList.add('active'); d.setAttribute('aria-hidden','false'); d.setAttribute('data-drawer-type',type||''); const panel=d.querySelector('.drawer-panel'); if(panel) panel.scrollTop=0; if(body) body.scrollTop=0;
  ...
  if(type==='quickAdd'||type==='transaction'){ const t=data||{}; const isSpend=(t.amount===undefined)||(Number(t.amount||0)<=0); title.textContent=data?'Edit transaction':'Add transaction'; sub.textContent='Record a purchase, payment, or deposit.'; body.innerHTML=`<div class="card drawer-form-card"><div class="tx-type-toggle" role="group" aria-label="Transaction type"><button type="button" class="btn btn-small${isSpend?' selected':''}" data-tx-type="spend" onclick="setTxType('spend')">💸 Spend</button><button type="button" class="btn btn-small${!isSpend?' selected':''}" data-tx-type="income" onclick="setTxType('income')">💰 Income</button></div><div class="form-row"><div><label>Amount</label><div class="money-input"><span>$</span><input class="input" id="qaAmount" type="number" inputmode="decimal" step="0.01" min="0" value="${Math.abs(Number(t.amount||0))||''}" placeholder="0.00" autocomplete="off"></div></div><div><label>Date</label><input class="input" id="qaDate" type="date" value="${t.date||new Date().toISOString().slice(0,10)}"></div></div><div class="form-field"><label>Description</label><input class="input" id="qaDesc" data-autofocus value="${escapeHtml(t.description||'')}" placeholder="Merchant, payee, or note" autocapitalize="words" autocorrect="off" autocomplete="off"></div><div class="form-row"><div><label>Category</label><input class="input" id="qaCat" list="categoryList" value="${escapeHtml(t.category||'Other')}" autocomplete="off"></div><div><label>Account</label><input class="input" id="qaAcct" value="${escapeHtml(t.account||'General')}" autocomplete="off"></div></div><div class="toggle"><div><b>Mark as transfer</b><br><span class="muted">Hides from spending totals</span></div><div class="switch ${t.hidden?'on':''}" id="qaHidden" onclick="this.classList.toggle('on')" aria-label="Hide as transfer"></div></div><div class="drawer-actions"><button class="btn btn-primary" onclick="saveQuickTransaction('${t.id||''}')">Save transaction</button><button class="btn" onclick="closeDrawer()">Cancel</button>${t.id?`<button class="btn btn-danger" onclick="deleteTransaction('${t.id}')">Delete</button>`:''}</div></div>`; }
  if(type==='budget'){ title.textContent=data?'Edit budget':'Add budget'; sub.textContent='Set a simple monthly category limit.'; body.innerHTML=`<div class="card drawer-form-card"><div class="form-field"><label>Category</label><input class="input" id="budCat" list="categoryList" value="${escapeHtml(data?.category||'')}" autocomplete="off"></div><div class="form-field"><label>Monthly limit</label><div class="money-input"><span>$</span><input class="input" id="budLimit" type="number" inputmode="decimal" step="0.01" min="0" value="${data?.limit||''}" placeholder="0.00"></div></div><div class="drawer-actions"><button class="btn btn-primary" onclick="saveBudget('${data?.id||''}')">Save budget</button><button class="btn" onclick="closeDrawer()">Cancel</button>${data?.id?`<button class="btn btn-danger" onclick="deleteTrackerItem('budgets','${data.id}',{closeDrawer:true})">Delete</button>`:''}</div></div>`; }
  if(type==='goal'){ title.textContent=data?'Edit goal':'Add goal'; sub.textContent='Track a target, deadline, priority, and pace.'; body.innerHTML=`<div class="card drawer-form-card"><div class="form-field"><label>Name</label><input class="input" id="goalName" data-autofocus value="${escapeHtml(data?.name||'')}" autocapitalize="words" autocorrect="off"></div><div class="form-row"><div><label>Type</label><input class="input" id="goalType" value="${escapeHtml(data?.type||'Savings')}" autocapitalize="words"></div><div><label>Priority</label><select id="goalPriority"><option ${data?.priority==='High'?'selected':''}>High</option><option ${!data?.priority||data?.priority==='Medium'?'selected':''}>Medium</option><option ${data?.priority==='Low'?'selected':''}>Low</option></select></div></div><div class="form-row"><div><label>Target</label><div class="money-input"><span>$</span><input class="input" id="goalTarget" type="number" inputmode="decimal" step="0.01" min="0" value="${data?.target||''}" placeholder="0.00"></div></div><div><label>Current</label><div class="money-input"><span>$</span><input class="input" id="goalCurrent" type="number" inputmode="decimal" step="0.01" min="0" value="${data?.current||0}" placeholder="0.00"></div></div></div><div class="form-row"><div><label>Due date</label><input class="input" id="goalDue" type="date" value="${data?.dueDate||''}"></div><div><label>Linked account</label><input class="input" id="goalLinked" value="${escapeHtml(data?.linkedAccount||'')}" placeholder="Optional" autocapitalize="words"></div></div><div class="form-field"><label>Notes</label><input class="input" id="goalNotes" value="${escapeHtml(data?.notes||'')}" placeholder="Optional"></div><div class="drawer-actions"><button class="btn btn-primary" onclick="saveGoal('${data?.id||''}')">Save goal</button><button class="btn" onclick="closeDrawer()">Cancel</button>${data?.id?`<button class="btn btn-danger" onclick="deleteTrackerItem('goals','${data.id}',{closeDrawer:true})">Delete</button>`:''}</div></div>`; }
